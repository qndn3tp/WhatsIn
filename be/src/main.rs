...
use crate::{config::config, crawler::Crawler, news_route::news_routers, swagger_docs::NewsDoc};
...
async fn main() {
	...
	let app: Router = Router::new().nest_service("/api/vi/external", news_routers());

	let swagger = Router::new().merge(SwaggerUi::new("/api/v1/external/docs").url("/news/openapi.json", NewsDoc::openapi()));
