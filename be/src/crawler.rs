...
use crate::{error::BatchError, enums::Category, domain::news::NewsRequest, repository::news::NewsRepo};
...
impl Crawler{
    ...
    pub fn set_up() -> Result<Self, BatchError>{
        ...
        let schedule_json: Value = serde_json::from_reader(f_reader)
        .map_err(|err| BatchError::SetUpFailed(Box::new(err)))?;
        
        serde_json::from_value::<Crawler>(schedule_json)
        .map_err(|err| BatchError::SetUpFailed(Box::new(err)))
    }
    ...
    fn update_schedule(&self) -> Result<(), BatchError>{
        ...
        .map_err(|err| BatchError::ParsingError(Box::new(err)))?;
    
        let mut file = File::create("./schedule.json")
        .map_err(|err| BatchError::FileError(Box::new(err)))?;
